...
#include "src/multiIntersectBed/include/GenomeFile.h"
#include "multiIntersectBed.h"
//#include "version.h"

#define VERSION "v2.29.2-35-g0712442-dirty"
...
int multiintersect_main(int argc, char* argv[])
{
    ...
    for(int i = 1; i < argc; i++) {
        ...
        if(PARAMETER_CHECK("-i", 2, parameterLength)) {
        ...
        else if(PARAMETER_CHECK("-examples", 11, parameterLength)) {
            multiintersect_help();
            ...
            if ((i+1) < argc) {
                ...
                streambuf* stream_buffer_file = output_file.rdbuf(); 
              
                // Redirect cout to file 
                cout.rdbuf(stream_buffer_file); 
            }else{
                cerr << "Error: missing output file name (-o)." << endl;
                exit(1);
            }
        }
    }
...
void multiintersect_help(void) {
    ...
    cerr << "\t-o\t\t"          << "specify output filename." << endl<<endl;
